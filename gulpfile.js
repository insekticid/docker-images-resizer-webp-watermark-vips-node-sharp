...
gulp.task('images:watch', function() {
    return gulp.watch(config.source_images_path, ['generate']).on('change', function(event) {
...
gulp.task("generate", function () {
    ...
    //.pipe(changed(config.images_path + "/270x190/"))
...
gulp.task('generate:watch', function() {
    return gulp.watch([config.images_path + "/design2016/*.png", config.images_path + "/themes/*.png"], ['images_convert']).on('change', function(event) {
        console.log('File ' + event.path + ' was ' + event.type + ', running tasks...');
    });
});
